...
from ttt.sk_inference import (
    TreeClassificationInference,
    TreeRegressionInference,
    ForestClassifierInference,
    ForestRegressorInference,
)
...
@pytest.fixture(
    scope="function",
    params=[
        (DecisionTreeClassifier(), TreeClassificationInference),
...
@pytest.fixture(
    scope="function",
    params=[
        (DecisionTreeRegressor(), TreeRegressionInference),
...
def test_classification(classifiers, binary_classification_dataset):
    ...
    with tf.Session():
        ...
        np.testing.assert_allclose(clf.predict_proba(x), predict_prob_res.eval({d: x}))
    ...
    with tf.Session():
        np.testing.assert_allclose(clf.predict(x), predict_res.eval({d: x}))
        np.testing.assert_allclose(reshape_proba(x), predict_prob_res.eval({d: x}))

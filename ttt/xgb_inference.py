...
def df_to_tree(df):
    ...
    children_left = np.array(
        [
            -1 if not isinstance(r, str) else int(r.split("-")[1])
            for r in meta_df["Yes"].values
        ]
    )
    children_right = np.array(
        [
            -1 if not isinstance(r, str) else int(r.split("-")[1])
            for r in meta_df["No"].values
        ]
    )

    features = np.array(
        [-2 if r == "Leaf" else int(r.strip("f")) for r in meta_df["Feature"].values]
    )
...
class RegressionInference(InferenceBase):
    def _predict(self, input_):
        return [
            tree_df_to_value(input_, tree_df) for tree_df in model_to_tree_dfs(self.clf)
        ]
...
class ClassificationInference(RegressionInference):
    def predict_proba(self, input_):
        ...
        if self.clf.n_classes_ <= 2:
            positive_pred = tf.sigmoid(tf.reduce_sum(predicted, axis=0))[:, tf.newaxis]
            return tf.concat([1 - positive_pred, positive_pred], axis=1)
        else:
            return tf.transpose(
                tf.nn.softmax(
                    tf.reduce_sum(
                        tf.split(predicted, self.clf.n_estimators, axis=0), axis=0
                    ),
                    axis=0,
                )
            )

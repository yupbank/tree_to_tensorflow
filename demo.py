...
def main(_):
    ...
    assert isinstance(
        tf_estimator, tf.contrib.tensor_forest.client.random_forest.TensorForestEstimator)

    feature_spec = {'featuresww': tf.FixedLenFeature([4], tf.float32)}
    ...
    print y[0], y[1]
    print json.dumps(
        dict(instances=[dict(features=X[0].tolist()), dict(features=X[1].tolist())]))
